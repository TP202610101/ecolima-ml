...
CAMBIOS v2 (2026-05-29):
  - Corregido offset del sigmoid: de -logit+1.5 a -logit+0.2
    El offset anterior comprimía todas las probabilidades hacia valores bajos,
    forzando un umbral de clasificación de 0.10 y predicción degenerada (todo clase 1).
  - Balance de clases objetivo: ~40% positivos (is_optimal=1), más realista
    para un problema de ubicación donde las zonas óptimas son minoría.
  - Aumentada la separabilidad entre clases: los coeficientes del logit
    ahora generan una distribución bimodal más clara en prob_optimal.
"""
...
def _generate_district_batch(
    ...
    walkability_score    = rng.beta(5, 2, n_samples)
    ...
    # ── Target ────────────────────────────────────────────────────────────────
    # FIX v2: offset reducido de +1.5 a +0.2 para centrar el sigmoid.
    # Coeficientes aumentados para mayor separabilidad entre clases.
    # Balance objetivo: ~40% positivos (zona óptima = minoría realista).
    #
    # Lógica de negocio:
    #   + Alta densidad poblacional → más demanda potencial
    #   + Alta accesibilidad        → zona alcanzable a pie
    #   + Alta brecha de cobertura  → zona que necesita el punto
    #   + NSE alto/medio            → mayor participación en reciclaje
    #   - Alta densidad de reciclaje existente → zona ya cubierta
    logit = (
          2.0 * (population_density / 25_000)       # normalizado al máximo ~Lima
        + 1.5 * accessibility_composite
        + 1.2 * coverage_gap_index
        + 0.8 * nse_high_ratio
        - 1.0 * (recycling_density_1km / 5).clip(0, 1)  # penaliza zonas ya cubiertas
        + rng.normal(0, 0.4, n_samples)                  # ruido reducido
    )
    # offset +0.2 → sigmoid centrado → ~40-45% positivos en promedio
    prob_optimal = 1 / (1 + np.exp(-logit + 0.2))
...
def generate_synthetic_dataset(
    ...
    print(f"[data_generator] Balance — is_optimal=1: {balance.get(1, 0):.1%}  is_optimal=0: {balance.get(0, 0):.1%}")
